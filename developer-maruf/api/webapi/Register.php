...
ini_set('display_errors', 0);
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['domainurl']) && isset($shonupost['invitecode']) && isset($shonupost['language']) && isset($shonupost['phonetype']) && isset($shonupost['pwd']) && isset($shonupost['random']) && isset($shonupost['registerType']) && isset($shonupost['signature']) && isset($shonupost['timestamp']) && isset($shonupost['username'])) {
			...
			/*
			 * Keep this in sync with the bundled frontend request interceptor:
			 * - timestamp/signature are not part of the digest
			 * - track and xosoBettingData are sent, but intentionally not signed
			 * - null/empty values are omitted
			 * - remaining keys are sorted before JSON + MD5
			 *
			 * The old fixed-field JSON omitted newer registration fields such as
			 * deviceId and packId, so an otherwise valid frontend request failed.
			 */
			$signaturePayload = $shonupost;
			unset(
				$signaturePayload['signature'],
				$signaturePayload['timestamp'],
				$signaturePayload['track'],
				$signaturePayload['xosoBettingData']
			);
			foreach ($signaturePayload as $key => $value) {
				if ($value === null || $value === '') {
					unset($signaturePayload[$key]);
				}
			}
			...
			if(hash_equals($shonusign, strtoupper(trim($signature)))){
...
$owncode = $codethiefstfu . $nextId;
						...
						// New registrations always start with zero cash and zero signup bonus.
