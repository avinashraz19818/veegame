...
error_reporting(E_ALL);
	...
	require_once __DIR__ . '/_bet_transactions_saas.php';
...
if (!function_exists('tx_frontend_signature')) {
    function tx_frontend_signature(array $payload): string
    {
...
date_default_timezone_set("Asia/Kolkata");
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['date']) && isset($shonupost['language']) && isset($shonupost['pageNo']) && isset($shonupost['pageSize']) && isset($shonupost['random']) && isset($shonupost['signature']) && isset($shonupost['timestamp']) && isset($shonupost['type'])) {
			...
			$shonusign = tx_frontend_signature($shonupost);
			if($shonusign == $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						...
						send_bet_transactions_with_saas_if_requested($conn, (int)$shonuid, (int)$type, (int)$pageNo, (int)$pageSize, (string)$date, $shnunc);
