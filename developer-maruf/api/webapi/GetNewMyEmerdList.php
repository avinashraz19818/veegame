<?php 
	...
	require_once __DIR__ . '/_saas_bet_history.php';
...
if (!function_exists('mh_frontend_signature')) {
    function mh_frontend_signature(array $payload): string
    {
...
date_default_timezone_set("Asia/Kolkata");
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['endDate']) && isset($shonupost['gameType']) && isset($shonupost['language']) && isset($shonupost['pageNo']) && isset($shonupost['pageSize']) && isset($shonupost['random']) && isset($shonupost['signature']) && isset($shonupost['startDate']) && isset($shonupost['timestamp']) && isset($shonupost['type'])) {
			...
			$shonusign = mh_frontend_signature($shonupost);
			if($shonusign == $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						...
						send_saas_bet_history_if_present($conn,(int)$shonuid,(int)$gameType,(int)$pageNo,(int)$pageSize,(string)$startDate,(string)$endDate);
