				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						...
						elseif($withdrawid == 2){
							$data['lastBandCarkName'] = null;
							$data['withdrawalslist'] = [];
							$stmt = $conn->prepare('SELECT id,upi_id,mobile,name FROM upi_withdrawal WHERE user_id=? ORDER BY id DESC');
							if ($stmt) {
								$stmt->bind_param('i', $shonuid); $stmt->execute(); $upiResult=$stmt->get_result();
								while($upi=$upiResult->fetch_assoc()) $data['withdrawalslist'][]=['bid'=>(int)$upi['id'],'bankCode'=>'upi','upiName'=>(string)$upi['name'],'upiAccount'=>(string)$upi['upi_id'],'bankName'=>'UPI','beneficiaryName'=>(string)$upi['name'],'accountNo'=>replaceWithAsterisks((string)$upi['upi_id']),'ifsCode'=>'','withType'=>2,'mobileNo'=>replaceWithAsterisks((string)$upi['mobile']),'bankProvince'=>'','bankCity'=>'','bankAddress'=>''];
								if(!empty($data['withdrawalslist']))$data['lastBandCarkName']=$data['withdrawalslist'][0]['beneficiaryName'];
								$stmt->close();
							}
						}
						elseif($withdrawid == 3){
...
$dailyLimit = ((int)$withdrawid === 3) ? 5 : 3;
$shelly_1 = max(0, $dailyLimit - $shelly);
...
$data["withdrawalsrule"]["minPrice"] = (float)app_setting('withdraw_min_amount',110);
$data["withdrawalsrule"]["maxPrice"] = (float)app_setting('withdraw_max_amount',50000);
...
$data["withdrawalsrule"]["amount"] = $balarr["motta"];
						...
						$turnover = withdrawal_turnover((int)$shonuid);
						$data["withdrawalsrule"]["amountofCode"] = $turnover['remaining'];
						$wiwo = $turnover['remaining'] <= 0 ? (float)$balarr["motta"] : 0;

				// Additional Data Assignments
				$data["withdrawalsrule"]["canWithdrawAmount"] = $wiwo;
				$data["withdrawalsrule"]["c2cUnitAmount"] = 0;
				$data["withdrawalsrule"]["uRate"] = (float)app_setting('usdt_inr_rate',103);
