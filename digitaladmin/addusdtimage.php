...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['uploadImage']) && isset($_FILES["image"])) {
    ...
    $check_query = mysqli_query($conn, "SELECT * FROM images_usdt WHERE filename = '$filename'");
    ...
    if ($uploadOk && move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        $insert = "INSERT INTO images_usdt (filename, status) VALUES ('$filename', '0')";
...
if (isset($_POST['upiid'])) {
    ...
    mysqli_query($conn, "UPDATE images_usdt SET status = '0'");
    $update = "UPDATE images_usdt SET status = '1' WHERE filename = '$filename'";
...
if (isset($_GET['delete'])) {
    ...
    $get_file = mysqli_fetch_assoc(mysqli_query($conn, "SELECT filename FROM images_usdt WHERE id = $id"));
    $file_path = "../images_usdt/" . $get_file['filename'];
    if (file_exists($file_path) && unlink($file_path)) {
        mysqli_query($conn, "DELETE FROM images_usdt WHERE id = $id");
...
<title>Add USDT channel Image</title>
                            ...
                            <div class="col-12 col-lg-8">
                                ...
                                <div class="card mb-4">
                                    <div class="card-header">
                                        <h5 class="mb-0">Upload USDT Image</h5>
                                    </div>
                                    <div class="card-body">
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="mb-0">Choose Active USDT Image</h5>
                                    </div>
                                    <div class="card-body">
                                        <form action="" method="post">
                                            <?php
                                            $sel_upi = "SELECT * FROM images_usdt";
