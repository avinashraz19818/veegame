...
<head>
    ...
    <title>Agent Total Active Users</title>
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <div class="card">
                            <div class="table-responsive text-nowrap">
                                ...
                                <h4 class="mx-5">Total Active Users Level <?=$level?></h4>
                                <table class="table" id="example1">
                                    ...
                                    <tbody class="table-border-bottom-0">
                                        ...
                                        $sql = "WITH AgentTeams AS (
                                                    ...
                                                    SELECT s.id AS team_member_id, 1 AS level
                                                    FROM shonu_subjects s WHERE s.code".($level-1==0?'':$level-1)." = '$ownCode'
                                                ), 
                                                AllBets AS (
                                                    -- Fetch bet details of team members only
                                                    SELECT DISTINCT b.byabaharkarta 
                                                    FROM (
                                                    ...
                                                    JOIN AgentTeams atm ON b.byabaharkarta = atm.team_member_id  -- Ensure bets belong to level 1 users
                                                ), 
                                                ActiveTeamMembers AS (
                                                    -- Get the details of active users at level 1
                                                    SELECT DISTINCT ss.id, ss.mobile
                                                    FROM shonu_subjects ss
                                                    JOIN thevani t ON t.balakedara = ss.id
                                                        AND t.motta >= 100  -- User must have deposited at least 100 INR
                                                        AND t.sthiti = 1
                                                    JOIN AllBets b ON b.byabaharkarta = ss.id  -- User must have placed at least one bet
                                                )
                                                -- Final SELECT statement
                                                SELECT * FROM ActiveTeamMembers;
