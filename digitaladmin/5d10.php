...
<body>
                        ...
                        <div class="card mb-5 p-5">
                            ...
                            <form class="row g-6 mb-6 justify-content-center" action="api/prediction-set.php" id="pre" method="POST">
                                <div class="col-sm-12 col-xl-12 col-lg-12 col-md-12 d-flex justify-content-center align-items-center">
                                    <input type="text" id="next" name="username" min="00000" max="99999"  minlength="5" maxlength="5" placeholder="Enter a number from 00000-99999" class="flex-grow-1 cool-input p-3" style="height: 40px;">
                                    <input type="hidden" name="type" value="5d10">
                                    <input type="hidden" name="issue_number" id="issue-number">
                                </div>
                                <div class="col-sm-12 col-xl-12 col-lg-12 col-md-12 d-flex justify-content-center align-items-center">
                                    <button class="btn btn-success w-100" onclick="sub(event)">Set prediction</button>
                                </div>
                            </form>
                        </div>
                        
                        <?php $adminGameType='5d10'; $adminHasBetDetails=false; require __DIR__.'/includes/live_big_small.php'; ?>
                        <div class="card mt-5">
	...
	function wingoonetotal()
	{
		...
		$.ajax({
		type: "Post",
		url: "api/total-bet-amount.php?periodid=" + periodid+"&type=5d10",
	...
	function refreshLiveBets() {
	    ...
	    let endpoint = "api/live-game-bets.php?type=5d10&periodid=" + encodeURIComponent(periodid);
    ...
    function sub(e) {
        var p = $("#next").val();
        // Check if input is exactly 5 digits (00000 - 99999)
        if (!/^\d{5}$/.test(p)) {
            alert("Error: Please enter a 5-digit number (00000 - 99999).");
            $("#next").focus();
            return;
        }else 
...
function startTimer(startTime, endTime) {
    ...
    function updateTimer() {
            ...
            if (distance < 1) {
                clearInterval(timer);
                // Auto-unset the prediction when timer expires
                resetman();
                fetchServerTime()
                    .then(data => {
    ...
    function fetchServerTime() {
        return fetch('api/servertime.php?type=5d10')
            .then(response => {
                if (!response.ok) {
