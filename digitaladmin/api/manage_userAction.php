...
if(isset($_POST['type'])){
	if($_POST['type']=='chk'){
		...
		$time=date( 'Y-m-d H:i:s' );
		$sqlA = "Update `khate` set sthiti = 1 where `byabaharkarta`='".$_POST['id']."' and kodprakara=7";
		mysqli_query($conn,$sqlA);
	}else if($_POST['type']=='unverify'){
		$time=date( 'Y-m-d H:i:s' );
		$sqlA = "Update `khate` set sthiti = 0 where `byabaharkarta`='".$_POST['id']."' and kodprakara=7";
		mysqli_query($conn,$sqlA);
	}	
}
?>