...
$Query = mysqli_query($conn, "SELECT 
            (SELECT SUM(h.motta) FROM hintegedukolli h 
             ...
             AND DATE(h.dinankavannuracisi) = '$date' 
             AND h.sthiti = 1) AS level1_total,
        
            (SELECT SUM(h.motta) FROM hintegedukolli h 
             JOIN shonu_subjects ss ON h.balakedara = ss.id 
             WHERE ss.code1 = '$ownCode' 
             AND DATE(h.dinankavannuracisi) = '$date' 
             AND h.sthiti = 1) AS level2_total,
        
            (SELECT SUM(h.motta) FROM hintegedukolli h 
             JOIN shonu_subjects ss ON h.balakedara = ss.id 
             WHERE ss.code2 = '$ownCode' 
             AND DATE(h.dinankavannuracisi) = '$date' 
             AND h.sthiti = 1) AS level3_total,
        
            (SELECT SUM(h.motta) FROM hintegedukolli h 
             JOIN shonu_subjects ss ON h.balakedara = ss.id 
             WHERE ss.code3 = '$ownCode' 
             AND DATE(h.dinankavannuracisi) = '$date' 
             AND h.sthiti = 1) AS level4_total,
        
            (SELECT SUM(h.motta) FROM hintegedukolli h 
             JOIN shonu_subjects ss ON h.balakedara = ss.id 
             WHERE ss.code4 = '$ownCode' 
             AND DATE(h.dinankavannuracisi) = '$date' 
             AND h.sthiti = 1) AS level5_total,
        
            (SELECT SUM(h.motta) FROM hintegedukolli h 
             JOIN shonu_subjects ss ON h.balakedara = ss.id 
             WHERE ss.code5 = '$ownCode' 
             AND DATE(h.dinankavannuracisi) = '$date' 
             AND h.sthiti = 1) AS level6_total;");
