...
// DB table to use 
//$table = 'tbl_user'; 
$periodid = isset($_GET['periodid']) ? $_GET['periodid'] : '(SELECT atadaaidi FROM gelluonduhogu_aidudi_drei ORDER BY gelluonduhogu_aidudi_drei.kramasankhye DESC LIMIT 1)';
$table = <<<EOT
	 (
		SELECT
		...
		WHERE bajikattuttate_aidudi_drei.kalaparichaya = '{$periodid}'
	 ) temp
	EOT;
