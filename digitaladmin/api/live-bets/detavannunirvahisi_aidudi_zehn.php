...
// DB table to use 
//$table = 'tbl_user'; 
$periodid = isset($_GET['periodid']) ? $_GET['periodid'] : '(SELECT atadaaidi FROM gelluonduhogu_aidudi_zehn ORDER BY gelluonduhogu_aidudi_zehn.kramasankhye DESC LIMIT 1)';
$table = <<<EOT
	 (
		SELECT
		...
		WHERE bajikattuttate_aidudi_zehn.kalaparichaya = '{$periodid}'
	 ) temp
	EOT;
