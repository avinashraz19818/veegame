...
// DB table to use 
//$table = 'tbl_user'; 
$periodid = isset($_GET['periodid']) ? $_GET['periodid'] : '(SELECT atadaaidi FROM gelluonduhogu_aidudi_funf ORDER BY gelluonduhogu_aidudi_funf.kramasankhye DESC LIMIT 1)';
$table = <<<EOT
	 (
		SELECT
		...
		WHERE bajikattuttate_aidudi_funf.kalaparichaya = '{$periodid}'
	 ) temp
	EOT;
