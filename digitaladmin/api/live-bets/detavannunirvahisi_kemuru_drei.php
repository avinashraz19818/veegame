...
// DB table to use 
//$table = 'tbl_user'; 
$periodid = isset($_GET['periodid']) ? $_GET['periodid'] : '(SELECT atadaaidi FROM gelluonduhogu_kemuru_drei ORDER BY gelluonduhogu_kemuru_drei.kramasankhye DESC LIMIT 1)';
$table = <<<EOT
	 (
		SELECT
		...
		WHERE bajikattuttate_kemuru_drei.kalaparichaya = '{$periodid}'
	 ) temp
	EOT;
