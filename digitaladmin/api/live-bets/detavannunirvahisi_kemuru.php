...
$periodid = isset($_GET['periodid']) ? $_GET['periodid'] : '(SELECT atadaaidi FROM gelluonduhogu_kemuru ORDER BY gelluonduhogu_kemuru.kramasankhye DESC LIMIT 1)';
...
$table = <<<EOT
	 (
		...
		WHERE bajikattuttate_kemuru.kalaparichaya = '{$periodid}'
	 ) temp
	EOT;
