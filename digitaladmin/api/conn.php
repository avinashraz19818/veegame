...
// PHP 8 may throw mysqli_sql_exception before the old !$conn check can run.
// Keep failures controlled so the admin login can show a useful message.
...
if (!$conn) {
	if (defined('ADMIN_LOGIN_REQUEST') && ADMIN_LOGIN_REQUEST === true) {
...
if (!defined('ADMIN_SKIP_RUNTIME_SCHEMA') || ADMIN_SKIP_RUNTIME_SCHEMA !== true) {
	require_once dirname(__DIR__, 2) . '/admin_runtime_schema.php';
}
...
function DB_selectOne($query, $params = [])
{
