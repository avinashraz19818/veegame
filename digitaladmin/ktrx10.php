...
// Check for existing TRX10 prediction in admin_predictions table
$munde = mysqli_query($conn, "SELECT prediction_number FROM `admin_predictions` WHERE `game_type`='ktrx10' AND `is_active`=TRUE ORDER BY created_at DESC LIMIT 1");
if ($munde && mysqli_num_rows($munde) > 0) {
    $uhisi = mysqli_fetch_array($munde);
    $uhisisankhye = $uhisi['prediction_number'];
} else {
...
<body>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="card mb-5 p-5">
                            ...
                            <form class="row g-6 mb-6 justify-content-center" action="api/prediction-set.php" id="pre" method="POST">
                                <div class="col-sm-12 col-xl-12 col-lg-12 col-md-12 d-flex justify-content-center align-items-center">
                                    <input type="number" id="next" name="username" min="0" max="9" placeholder="Enter a number from 0-9" class="flex-grow-1 cool-input p-3" style="height: 40px;">
                                    <input type="hidden" name="type" value="ktrx10">
                        ...
                        <?php $adminGameType='ktrx10'; $adminHasBetDetails=false; require __DIR__.'/includes/live_big_small.php'; ?>
                        <div class="card mt-5">
	...
	function wingoonetotal()
	{
		...
		$.ajax({
		type: "Post",
		url: "api/total-bet-amount.php?periodid=" + periodid+"&type=ktrx10",
	...
	function refreshLiveBets() {
	    ...
	    let endpoint = "api/live-game-bets.php?type=ktrx10&periodid=" + encodeURIComponent(periodid);
	    if (window.adminBigSmallFilter) endpoint += "&betfilter=" + encodeURIComponent(window.adminBigSmallFilter);
	    if (liveBetTable) { liveBetTable.ajax.url(endpoint).load(null, !!arguments[0]); return; }
	    liveBetTable = $('#example2').DataTable({ processing:true, serverSide:true, ajax:endpoint, paging:true, lengthChange:false, searching:false, ordering:false, info:true, autoWidth:false, pageLength:50, dom:'t<"row p-5"<"col-sm-12 col-md-6"i><"col-sm-12 col-md-6"p>>' });
	}
...
function startTimer(startTime, endTime) {
    ...
    function fetchServerTime() {
        return fetch('api/servertime.php?type=ktrx10')
            .then(response => {
                if (!response.ok) {
