...
try {
    $pendingOverride = sl_admin_override_get_pending($conn, 'WinGo_3M');
...
<body>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <?php $adminGameType='wingo3'; $adminHasBetDetails=true; require __DIR__.'/includes/live_big_small.php'; ?>
                        <div class="card mt-5">
                        ...
                        <form action="api/prediction-set.php" id="pre" method="POST">
                            ...
                            <input type="hidden" name="type" value="wingo3">
	...
	function getbettingdata(actiontype)
	{
		let periodid=document.getElementById('curr-period').innerHTML;
			$.ajax({
			type: "GET",
			url: "api/game-bet-summary.php?type=wingo3&periodid=" + encodeURIComponent(periodid),
			dataType: "json",
			success: function (response) {
			    const rows = (response.data || []).map(function(item) { return '<tr><td>' + item.bet + '</td><td>₹ ' + Number(item.stake).toFixed(2) + '</td><td>' + item.users + '</td><td>₹ ' + Number(item.potentialPayout).toFixed(2) + '</td></tr>'; }).join('');
			    document.getElementById("betdetail").innerHTML = rows || '<tr><td colspan="4" class="text-center">No bets in this period</td></tr>';
			    return false;
			  },
			  error: function (e) {}
			  });
	...
	function wingoonetotal()
	{
		...
		$.ajax({
		type: "Post",
		url: "api/total-bet-amount.php?periodid=" + periodid+"&type=wingo3",
	...
	function refreshLiveBets() {
	    ...
	    let endpoint = "api/live-game-bets.php?type=wingo3&periodid=" + encodeURIComponent(periodid);
...
function startTimer(startTime, endTime) {
    ...
    function fetchServerTime() {
        return fetch('api/servertime.php?type=wingo3')
            .then(response => {
                if (!response.ok) {
