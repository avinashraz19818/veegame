...
if(isset($_POST['update'])) {
    ...
    $khatehesaru = mysqli_real_escape_string($conn, $_POST['khatehesaru']);
    
    $update_sql = "UPDATE khate 
                   SET khatesankhye = '$khatesankhye', 
                       phalanubhavi = '$phalanubhavi', 
                       kod = '$kod', 
                       khatehesaru = '$khatehesaru' 
                   WHERE shonu = '$shonu'";
...
if(isset($_POST['delete'])) {
    $shonu = mysqli_real_escape_string($conn, $_POST['id']);
    $delete_sql = "DELETE FROM khate WHERE shonu = '$shonu'";
    if(mysqli_query($conn, $delete_sql)) {
        ...
        echo '<script type="text/JavaScript">alert("Failed to delete bank account!");</script>';
    }
}
?>
        ...
        <div class="layout-page">
            ...
            <div class="content-wrapper">
                <div class="container-xxl flex-grow-1 container-p-y">
                    ...
                    <h4 class="mb-4">Manage Bank Accounts</h4>

                    <!-- Search User Card -->
                    <div class="card mb-4">
                        <div class="card-header">
                            <h5 class="card-title mb-0">Search User Bank Details</h5>
                        </div>
                        <div class="card-body">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title mb-0">
                                Bank Accounts for User: <?= htmlspecialchars($searched_userid) ?>
                        ...
                        <div class="card-body">
                            <?php while ($row = mysqli_fetch_assoc($user_records)): ?>
                                <div class="bank-row" id="row-<?= $row['shonu'] ?>">
                                    <div class="bank-info">
                                        <div class="d-flex align-items-center mb-2">
                                            <h6 class="mb-0 me-3"><?= htmlspecialchars($row['phalanubhavi']) ?></h6>
                                            <span class="bank-type type-bank">
                                                <?= strtoupper($row['kodprakara']) ?>
                                            </span>
                                        </div>
                                        <p class="mb-1 text-muted">
                                            <strong>Account No:</strong> <?= htmlspecialchars($row['khatesankhye']) ?>
                                        </p>
                                        <p class="mb-1 text-muted">
                                            <strong>Bank Name:</strong> <?= htmlspecialchars($row['khatehesaru']) ?>
                                        </p>
                                        <p class="mb-1 text-muted">
                                            <strong>IFSC Code:</strong> <?= htmlspecialchars($row['kod']) ?>
                                        </p>
                                        <p class="mb-0 text-muted small">
                                            <strong>ID:</strong> <?= $row['shonu'] ?> | 
                                            <strong>Account Type:</strong> <?= $row['khatakrama'] ?>
                                        </p>
                                    </div>
                                    <div class="bank-actions">
                                        <button type="button" class="btn btn-sm btn-warning" 
                                                onclick="toggleEditForm(<?= $row['shonu'] ?>)">
                                            <i class="ri-edit-line me-1"></i> Edit
                                        </button>
                                        <form method="post" action="" style="display: inline;">
                                            <input type="hidden" name="id" value="<?= $row['shonu'] ?>">
                                ...
                                <!-- Edit Form -->
                                <div class="edit-form" id="edit-form-<?= $row['shonu'] ?>">
                                    <form method="post" action="">
                                        <input type="hidden" name="id" value="<?= $row['shonu'] ?>">
                                        <input type="hidden" name="userid" value="<?= $searched_userid ?>">
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control" name="khatesankhye" 
                                                       value="<?= htmlspecialchars($row['khatesankhye']) ?>" required>
                                            </div>
                                        </div>
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                <label class="form-label">Bank Name</label>
                                                <input type="text" class="form-control" name="khatehesaru" 
                                                       value="<?= htmlspecialchars($row['khatehesaru']) ?>" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">IFSC Code</label>
                                                <input type="text" class="form-control" name="kod" 
                                                       value="<?= htmlspecialchars($row['kod']) ?>" required>
                                            </div>
                                        </div>
                                        <div class="row">
