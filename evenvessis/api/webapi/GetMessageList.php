...
while ($notificationRow = $notificationResult->fetch_assoc()) {
    $message = "Your account is logged in {$notificationRow['created_at']}";
    
    // Check if it's a registration message
    if ($notificationRow['title'] == "NEW MEMBER") {
    ...
    if ($notificationRow['title'] == "Notification of cash withdrawal") {
   ...
   if ($notificationRow['title'] == "Recharge Arrival") {
    $amountQuery = mysqli_query($conn, "SELECT motta FROM thevani WHERE balakedara = '".$notificationRow['user_id']."' ORDER BY dinankavannuracisi DESC LIMIT 1");
    $amountRow = mysqli_fetch_array($amountQuery);
    $amount = $amountRow['motta'];

    $message = "Your account has arrived $amount Rs";
   }
   
   if ($notificationRow['title'] == "APPLY FOR WITHDRAWAL") {
    $amountQuery = mysqli_query($conn, "SELECT motta FROM thevani WHERE balakedara = '".$notificationRow['user_id']."' ORDER BY dinankavannuracisi DESC LIMIT 1");
    $amountRow = mysqli_fetch_array($amountQuery);
    $amount = $amountRow['motta'];
