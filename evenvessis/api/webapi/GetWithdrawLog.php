            ...
            if ($authData['status'] === 'Success') {
                ...
                if (mysqli_num_rows($checkResult) === 1) {
                    try {
                        ...
                        // Fix condition for all withdrawal types
                        $condition = "";
                        if ($withdrawid == 1) {
                            $condition = "AND madari = 1";
                        } elseif ($withdrawid == -1) {
                            $condition = "";
                        } elseif ($withdrawid == 23) {
                            $condition = "AND madari = 23"; // EasyPaisa
                        } elseif ($withdrawid == 24) {
                            $condition = "AND madari = 24"; // JazzCash
                        } elseif ($withdrawid == 3) {
                            $condition = "AND madari = 3"; // USDT
                        }
                        ...
                        if ($result) {
                            ...
                            while ($withdrawRow = $result->fetch_assoc()) {
                                $type = (int)$withdrawRow['madari'];
                                $withdrawName = '';
                                
                                // Determine withdrawal name based on type - FIXED
                                switch ($type) {
                                    case 1:
                                        $withdrawName = 'BANK_CARD'; // Bank withdrawal
                                        break;
                                    case 23:
                                        $withdrawName = 'EASYPAISA';
                                        break;
                                    case 24:
                                        $withdrawName = 'JAZZCASH';
                                        break;
                                    case 3:
                                        $withdrawName = 'USDT';
                                        break;
                                ...
                                $withdrawalslist[$i]['type'] = $type;
                                $withdrawalslist[$i]['withdrawNumber'] = $withdrawRow['dharavahi'];
                                $withdrawalslist[$i]['withdrawName'] = $withdrawName;
