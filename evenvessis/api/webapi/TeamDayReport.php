					...
					if($sesnum == 1){
						...
						if($shonupost['userId'] == ''){
							if($lv == -1){
								...
								$lvxfrtrchquery = "SELECT COUNT(DISTINCT t1.balakedara) as first_count
								  FROM thevani t1
								  WHERE t1.sthiti = '1' AND DATE(t1.dinankavannuracisi) = DATE('".$day."') AND 
								  t1.balakedara IN (SELECT id FROM shonu_subjects WHERE code = '".$sesarr['owncode']."' OR code1 = '".$sesarr['owncode']."' OR code2 = '".$sesarr['owncode']."' OR code3 = '".$sesarr['owncode']."' OR code4 = '".$sesarr['owncode']."' OR code5 = '".$sesarr['owncode']."')
								  AND (
								    SELECT COUNT(*) FROM thevani t2 
								    WHERE t2.balakedara = t1.balakedara AND t2.sthiti = '1' 
								    AND t2.dinankavannuracisi < t1.dinankavannuracisi
								  ) = 0";
								$lvxfrtrchresult = $conn->query($lvxfrtrchquery);
								$lvxfrtrchresult_ar = mysqli_fetch_array($lvxfrtrchresult);
								$data['data']['firstRecahrgeCount'] = (int)$lvxfrtrchresult_ar['first_count'];
								
								$lvxfrtrchquery_amt = "SELECT SUM(t1.motta) as sumotta
								  FROM thevani t1
								  WHERE t1.sthiti = '1' AND DATE(t1.dinankavannuracisi) = DATE('".$day."') AND 
								  t1.balakedara IN (SELECT id FROM shonu_subjects WHERE code = '".$sesarr['owncode']."' OR code1 = '".$sesarr['owncode']."' OR code2 = '".$sesarr['owncode']."' OR code3 = '".$sesarr['owncode']."' OR code4 = '".$sesarr['owncode']."' OR code5 = '".$sesarr['owncode']."')
								  AND (
								...
								$lvxfrtrchresult_amt = $conn->query($lvxfrtrchquery_amt);
								$lvxfrtrchresult_ar = mysqli_fetch_array($lvxfrtrchresult_amt);
								$sumotta_y = $lvxfrtrchresult_ar['sumotta'];
								$sumotta_y == null ? $data['data']['firstRecahrgeAmountSum'] = 0 : $data['data']['firstRecahrgeAmountSum'] = (float)$sumotta_y;
