...
$SL_CONFIG = require __DIR__ . '/config.php';
require_once dirname(__DIR__) . '/scripthub/conn.php';
require_once dirname(__DIR__) . '/scripthub/functions2.php';
require_once dirname(__DIR__) . '/scripthub/app_core.php';
require_once dirname(__DIR__) . '/scripthub/vip_core.php';
...
function sl_install_schema()
{
    ...
    if (!app_column_exists('saas_lottery_bets', 'tax_fee')) {
        if (!$conn->query("ALTER TABLE saas_lottery_bets ADD COLUMN tax_fee DECIMAL(18,4) NOT NULL DEFAULT 0 AFTER payout")) {
            throw new RuntimeException('Unable to add SaaS lottery tax column: ' . $conn->error);
        }
    }
...
{
    if (!is_array($payload) || sl_game_family($gameCode) !== 'WinGo') {
        return $payload;
    }
...
{
    if (sl_game_family($gameCode) !== 'WinGo' || !$list) {
        return $list;
    }
...
function sl_provider_current($gameCode)
{
    ...
    return sl_local_current($gameCode);
}
...
/**
 * Maps a SaaS game code to the legacy admin manual-override table.
 * Admin sets sthiti=1 for the chosen number via the WinGo/TrxWinGo
 * admin pages (itticina_geluvu*.php). sl_save_and_settle_results()
 * reads this ONCE per call, applies it to the FIRST new period in the
 * batch, then resets sthiti=0 so subsequent periods use real-feed.
 */
...
function sl_save_and_settle_results($gameCode, $list)
{
    ...
    // If admin has set a number via the WinGo admin panel, that number
    // will be injected into the FIRST new (unsaved) period in this batch,
    // completely replacing the real-feed (AR014) result for that period.
    // After applying, sthiti is reset to 0 automatically so that all
    // subsequent periods resume normal SaaS real-feed mode.
    // WinGo uses an exact issue-bound override. Keep the old one-shot
    // fallback only for legacy TrxWinGo admin pages.
    ...
    foreach ($normalized as $issue => $item) {
        ...
        if (isset($existing[$issue])) {
            // Period already settled — NEVER modify. Skip settlement re-trigger too.
            continue;
        }
        ...
        if ($adminOverrideNumber !== null) {
            ...
            $number          = $adminOverrideNumber;
            $color           = sl_result_color((int)$adminOverrideNumber);
            $sum             = 0;
            $item['premium'] = $premium;
            $item['number']  = $number;
            $item['color']   = $color;
            $item['sum']     = $sum;
            error_log('[saas-lottery] admin override injected for exact period: game=' . $gameCode . ' issue=' . $issue . ' number=' . $adminOverrideNumber);
        }

        // INSERT IGNORE — safe for concurrent polls. First writer wins.
        ...
        $insert->close();
        if (!$inserted) {
            // Another concurrent request already saved this period — read what was stored
    ...
    // === AUTO-CLEAR OVERRIDE AFTER USE ===
    // Once applied to one period, immediately reset sthiti=0 so the game
    // returns to normal SaaS real-feed mode for all subsequent periods.
...
{
    // Same Trend/result synchronization is a required service. It must not
    // be disabled by an admin flag or a stale settings row.
...
function sl_cached_history($gameCode, $limit = 100)
{
    ...
    if ($currentIssue !== '') {
        ...
        $stmt->bind_param('ssi', $gameCode, $currentIssue, $limit);
    } else {
...
function sl_history_page($gameCode, $input = array())
{
    ...
    $stmt->close();
    // Final response-boundary guard for upgraded databases that still contain
    // historical WinGo rows labelled from an Asia/Kolkata midnight.
